#!/bin/bash
#
# (1) Select projects and SHAs for evaluation (FROM GIVEN SHA TO NEWEST SHA)
# Usage: get_project_revisions_future.sh <revisions> <repo> <sha> <output-dir>
...
function check_input() {
  if [[ -z ${REVISIONS} || -z ${REPO} || -z ${SHA} || -z ${OUTPUT_DIR} ]]; then
    echo "Usage bash get_project_revisions_future.sh <revisions> <repo> <sha> <output-dir>"
    exit 1
  fi
...
function get_project() {
  ...
  local failure=0
  for commit in $(git rev-list --first-parent --topo-order --remove-empty --no-merges --branches=master -n 500 ${SHA}..HEAD | tac); do
